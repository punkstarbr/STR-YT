...
with open('./LISTAMASTER.m3u', 'w') as f:
    ...
    title = subprocess.run(['yt-dlp', '--get-title', 'youtube.com/videos'], stdout=subprocess.PIPE)
    title = title.stdout.decode().strip()
    thumbnail = subprocess.run(['yt-dlp', '--get-thumbnail', 'https://www.youtube.com/videos'], stdout=subprocess.PIPE)
